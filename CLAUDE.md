...
### Windows Notes
- `uvloop` is not supported on Windows; the bot now skips it when unavailable.
- Use `uv run src/bot_runner.py` if `pump_bot` isn't on PATH.
- Console logging is ASCII-safe to avoid Unicode errors on Windows terminals.
